...
with st.form("career_assessment_form"):
    ...
    for start in range(0, len(QUESTIONS), 2):
        ...
        for col, question in zip(cols, QUESTIONS[start:start+2]):
            with col:
                st.markdown(f"<div style='padding:14px 16px 8px;border-radius:16px 16px 0 0;background:rgba(255,255,255,.026);border:1px solid rgba(255,255,255,.055);border-bottom:none;'><div style='color:#7f8ba5;font-size:10px;font-weight:850;letter-spacing:.13em;'>{question['skill'].upper()}</div><div style='color:#f1f3fb;font-size:15px;font-weight:800;margin-top:6px;'>{question['question']}</div></div>", unsafe_allow_html=True)
